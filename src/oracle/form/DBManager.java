...
class DBManager {
  ...
  public static int deleteStudent(String userId, String passwd) {
   ...
   public  static int update1(Exam1 s) {
    ...
    try {
        Connection con = DriverManager.getConnection(DBUrl,DBUser,DBPassword);
        PreparedStatement st1 = con.prepareStatement("SELECT * FROM Course WHERE CourseID = ?");
        st1.setString(1, courseId);
        ResultSet rs = st1.executeQuery();
        if(rs.next()) {
            flag = false; // Course with the same ID already exists
        } else {
            PreparedStatement st = con.prepareStatement("INSERT INTO Course (CourseID, CourseName) VALUES (?, ?)");
            st.setString(1, courseId);
            st.setString(2, courseName);
            st.executeUpdate();
            flag = true; // Course added successfully
            st.close();
        }
        rs.close();
        st1.close();
